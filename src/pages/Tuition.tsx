...
import { DollarSign, Clock, Zap, Infinity, PieChart, TrendingUp, Calculator } from "lucide-react";
import { ReactNode, useState } from "react";

export default function Tuition() {
  const [tokens, setTokens] = useState(10); // in millions
  const standardCost = 3250;
  const savings = Math.max(0, (tokens * 0.5) - standardCost);
    ...
    <main className="pt-32 pb-24 bg-university-paper">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <section className="mb-32">
          <div className="bg-university-navy text-university-paper p-8 md:p-20 relative overflow-hidden">
            ...
            <div className="relative z-10 grid lg:grid-cols-2 gap-16 items-center">
              <div>
                ...
                <p className="text-white/60 mb-12 max-w-lg leading-relaxed">
                  Compare the cost of VNU's Epoch model against standard pay-as-you-go API costs for a high-intensity learning model processing millions of tokens.
                </p>
                
                <div className="space-y-8">
                  <div>
                    <label className="block text-[10px] uppercase tracking-widest font-bold text-university-gold mb-4">
                      Estimated Token Consumption (M / Epoch)
                    </label>
                    <input 
                      type="range" 
                      min="1" 
                      ...
                      className="w-full h-1 bg-white/20 appearance-none cursor-pointer accent-university-gold"
                    />
                    <div className="flex justify-between mt-4 font-mono text-sm text-white/40">
                      <span>1M</span>
                      <span className="text-university-gold font-bold">{tokens}M Tokens</span>
                      <span>50M</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-white/5 backdrop-blur-sm border border-white/10 p-10 space-y-12">
                <div className="grid grid-cols-2 gap-8">
                  <div>
                    <div className="text-[10px] uppercase tracking-widest text-white/40 mb-2">VNU Flat Rate</div>
                    <div className="text-3xl font-serif text-white">${standardCost.toLocaleString()}</div>
                  </div>
                  <div>
                    <div className="text-[10px] uppercase tracking-widest text-white/40 mb-2">Estimated API Cost</div>
                    <div className="text-3xl font-serif text-white/70">${(tokens * 0.5).toLocaleString()}</div>
                  </div>
                </div>

                <div className="pt-8 border-t border-white/10">
                   <div className="text-5xl font-serif text-university-gold mb-2">
                     ${savings.toLocaleString()} <span className="text-sm font-sans uppercase tracking-[0.2em] text-white/50">Savings</span>
                   </div>
                   <p className="text-xs text-white/40 italic">Calculated based on average tier-1 API market pricing of $0.50/1k tokens.</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="bg-white p-12 md:p-20 border border-university-navy/5 mb-32">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div className="bg-university-navy p-12 text-university-paper flex flex-col justify-between">
            <div>
            ...
            <button className="self-start text-xs font-bold uppercase tracking-widest text-university-gold hover:text-white border-b border-university-gold pb-1 transition-all">
              Apply for Compute Grant
            </button>
          </div>
          <div className="bg-university-crimson p-12 text-university-paper flex flex-col justify-between">
            <div>
            ...
            <button className="self-start text-xs font-bold uppercase tracking-widest text-white hover:text-university-gold border-b border-white pb-1 transition-all">
...
function PricingTier({ status, price, period, features, featured = false }: { status: string, price: string, period: string, features: string[], featured?: boolean }) {
  return (
    <div className={`p-10 flex flex-col h-full border transition-all duration-500 hover:shadow-2xl ${featured ? 'border-university-crimson bg-university-navy text-university-paper scale-105 z-10 shadow-xl' : 'border-university-navy/10 bg-white text-university-navy'}`}>
      ...
      <button className={`w-full py-4 uppercase tracking-widest text-xs font-bold transition-all ${featured ? 'bg-university-gold text-university-navy hover:bg-white' : 'bg-university-navy text-white hover:bg-university-crimson'}`}>
        Secure Enrollment
      </button>
    </div>
  )
